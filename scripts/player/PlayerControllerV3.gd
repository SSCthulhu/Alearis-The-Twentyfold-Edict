...
# World1 Ski Sequence
# -----------------------------
var _ski_active: bool = false
var _ski_transition_active: bool = false
var _ski_completed: bool = false
var _ski_speed: float = 0.0
var _ski_end_x: float = 22900.0
var _ski_jump_used: bool = false
var _ski_destination: Node2D = null
var _ski_fade_rect: ColorRect = null
var _ski_fade_out_time: float = 1.0
var _ski_fade_in_time: float = 1.0
var _ski_roll_speed_multiplier: float = 2.0
var _ski_ground_grace_time: float = 0.12
var _ski_ground_grace_timer: float = 0.0
var _ski_roll_loop_start_ratio: float = 0.05
var _ski_roll_loop_end_ratio: float = 0.58
var _ski_jump_buffer_time: float = 0.14
var _ski_jump_buffer_timer: float = 0.0
var _ski_jump_gravity: float = 3200.0
var _ski_fall_velocity: float = 2200.0
var _ski_roll_enabled: bool = false
var _ski_camera_target_offset: Vector2 = Vector2(700.0, 260.0)
var _ski_camera_saved_offset: Vector2 = Vector2.ZERO
var _ski_camera_saved_valid: bool = false
var _ski_camera_saved_smoothing_enabled: bool = true
var _ski_post_transition_input_block_time: float = 0.1
var _ski_post_transition_input_block_timer: float = 0.0
...
func _physics_process(delta: float) -> void:
	...
	# World1 one-way ski sequence. This bypasses normal movement/combat state handling.
	if _ski_active:
		_process_world1_ski(delta)
		move_and_slide()
		return

	if _ski_post_transition_input_block_timer > 0.0:
		_ski_post_transition_input_block_timer = maxf(_ski_post_transition_input_block_timer - delta, 0.0)
...
func _process_world1_ski(delta: float) -> void:
	var cam: Camera2D = get_node_or_null("Camera2D") as Camera2D
	# Keep camera tightly locked to player during ski for smooth look-ahead.
	if cam != null:
		cam.global_position = global_position
		if cam.has_method("reset_smoothing"):
			cam.call("reset_smoothing")

	# During transition/fades, freeze all motion completely.
	if _ski_transition_active:
		velocity = Vector2.ZERO
		return

	# Keep visuals in roll while auto-sliding and while jumping.
	if _ski_roll_enabled:
		_ensure_world1_ski_roll_animation()
		_update_world1_ski_roll_loop_window()
	_set_facing_direction(1.0)

	# Preserve normal jump/fall feel while forcing rightward movement.
	if not is_on_floor():
		velocity.y = move_toward(velocity.y, _ski_fall_velocity, _ski_jump_gravity * delta)
		_ski_ground_grace_timer = maxf(_ski_ground_grace_timer - delta, 0.0)
	else:
		if velocity.y > 0.0:
			velocity.y = 0.0
		_ski_ground_grace_timer = _ski_ground_grace_time
		# Allow repeat single jumps from ground only (no double jump).
		_ski_jump_used = false

	if Input.is_action_just_pressed(input_jump):
		_ski_jump_buffer_timer = _ski_jump_buffer_time
	else:
		_ski_jump_buffer_timer = maxf(_ski_jump_buffer_timer - delta, 0.0)

	if not _ski_transition_active:
		if _ski_jump_buffer_timer > 0.0 and _ski_ground_grace_timer > 0.0 and not _ski_jump_used and global_position.x < _ski_end_x:
			velocity.y = JUMP_VELOCITY
			_ski_jump_used = true
			_ski_ground_grace_timer = 0.0
			_ski_jump_buffer_timer = 0.0

		velocity.x = _ski_speed
	else:
		velocity.x = 0.0

	if not _ski_transition_active and global_position.x >= _ski_end_x:
		_start_world1_ski_transition()


func _ensure_world1_ski_roll_animation() -> void:
	if _body_3d_view == null:
		return

	var anim_player: AnimationPlayer = _body_3d_view.get("_anim_player") as AnimationPlayer
	if anim_player == null:
		return

	const ROLL_ANIM: StringName = &"QAnim/Roll"
	if not anim_player.has_animation(ROLL_ANIM):
		return

	# Match requested "double speed" feel during ski.
	anim_player.speed_scale = _get_default_anim_speed_scale() * _ski_roll_speed_multiplier

	if not anim_player.is_playing() or anim_player.current_animation != String(ROLL_ANIM):
		anim_player.play(ROLL_ANIM)
		var roll_anim: Animation = anim_player.get_animation(ROLL_ANIM)
		if roll_anim != null:
			anim_player.seek(roll_anim.length * _ski_roll_loop_start_ratio, true)


func _update_world1_ski_roll_loop_window() -> void:
	if _body_3d_view == null:
		return
	...
	const ROLL_ANIM: StringName = &"QAnim/Roll"
	if anim_player.current_animation != String(ROLL_ANIM):
		return
	if not anim_player.has_animation(ROLL_ANIM):
		return

	var roll_anim: Animation = anim_player.get_animation(ROLL_ANIM)
	if roll_anim == null or roll_anim.length <= 0.0:
		return

	var loop_start: float = roll_anim.length * _ski_roll_loop_start_ratio
	var loop_end: float = roll_anim.length * _ski_roll_loop_end_ratio
	if loop_end <= loop_start:
		return

	if anim_player.current_animation_position >= loop_end:
		anim_player.seek(loop_start, true)


func _start_world1_ski_transition() -> void:
	if _ski_transition_active:
		return
	_ski_transition_active = true
	_ski_jump_used = true
	velocity = Vector2.ZERO
	set_input_locked(true)
	call_deferred("_run_world1_ski_transition")


func _run_world1_ski_transition() -> void:
	if _ski_fade_rect != null:
		_ski_fade_rect.visible = true
		var c0: Color = _ski_fade_rect.color
		c0 = Color(1.0, 1.0, 1.0, 0.0)
		_ski_fade_rect.color = c0

		var tween_out: Tween = create_tween()
		tween_out.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
		tween_out.tween_method(_set_world1_ski_fade_alpha, 0.0, 1.0, _ski_fade_out_time)
		await tween_out.finished
	else:
		await get_tree().create_timer(_ski_fade_out_time).timeout

	# Stop roll visual before fade-in so character does not roll in place.
	_ski_roll_enabled = false
	if _body_3d_view != null:
		var anim_player: AnimationPlayer = _body_3d_view.get("_anim_player") as AnimationPlayer
		if anim_player != null and anim_player.is_playing():
			anim_player.stop()

	if _ski_destination != null and is_instance_valid(_ski_destination):
		global_position = _ski_destination.global_position

	# Snap camera with the player during the white frame to avoid visible travel.
	var cam: Camera2D = get_node_or_null("Camera2D") as Camera2D
	if cam != null:
	...
	velocity = Vector2.ZERO
	if cam != null and _ski_camera_saved_valid:
		cam.offset = _ski_camera_saved_offset

	# Ensure there is no residual movement/animation before fade-in starts.
	if _body_3d_view != null:
		var anim_player: AnimationPlayer = _body_3d_view.get("_anim_player") as AnimationPlayer
		if anim_player != null and anim_player.is_playing():
			anim_player.stop()
	...
	if _ski_fade_rect != null:
		...
		tween_in.tween_method(_set_world1_ski_fade_alpha, 1.0, 0.0, _ski_fade_in_time)
		await tween_in.finished
		_ski_fade_rect.visible = false
	else:
		await get_tree().create_timer(_ski_fade_in_time).timeout

	_ski_active = false
	_ski_transition_active = false
	_ski_completed = true
	_ski_jump_used = false
	_ski_jump_buffer_timer = 0.0
	velocity = Vector2.ZERO
	set_input_locked(false)
	_ski_post_transition_input_block_timer = _ski_post_transition_input_block_time
	if cam != null:
		cam.position_smoothing_enabled = _ski_camera_saved_smoothing_enabled
		if cam.has_method("reset_smoothing"):
			cam.call("reset_smoothing")
	if is_on_floor():
...
func _set_world1_ski_fade_alpha(alpha: float) -> void:
	if _ski_fade_rect == null:
		return
	var c: Color = _ski_fade_rect.color
	c.a = alpha
	_ski_fade_rect.color = c


func _get_default_anim_speed_scale() -> float:
...
func start_world1_ski_sequence(
	speed_multiplier: float,
	end_x: float,
	destination: Node2D,
	fade_rect: ColorRect
) -> void:
	"""Begin one-way World1 ski segment (called by FloorProgressionController)."""
	if _ski_active or _ski_completed:
		return

	_ski_active = true
	_ski_transition_active = false
	_ski_completed = false
	_ski_jump_used = false
	_ski_end_x = end_x
	_ski_destination = destination
	_ski_fade_rect = fade_rect
	_ski_speed = WALK_VELOCITY * _get_speed_multiplier() * maxf(speed_multiplier, 0.1)
	_ski_ground_grace_timer = _ski_ground_grace_time
	_ski_jump_buffer_timer = 0.0
	_ski_roll_enabled = true
	var cam: Camera2D = get_node_or_null("Camera2D") as Camera2D
	if cam != null:
		_ski_camera_saved_offset = cam.offset
		_ski_camera_saved_valid = true
		_ski_camera_saved_smoothing_enabled = cam.position_smoothing_enabled
		cam.offset = _ski_camera_target_offset
		cam.position_smoothing_enabled = false
		if cam.has_method("reset_smoothing"):
			cam.call("reset_smoothing")
	_ensure_world1_ski_roll_animation()

	# Cancel combat-oriented movement states so sequence control is deterministic.
	if active_state == STATE.ROLL \
	or active_state == STATE.LIGHT_ATTACK \
	or active_state == STATE.HEAVY_ATTACK \
	or active_state == STATE.ULTIMATE \
	or active_state == STATE.DEFEND:
